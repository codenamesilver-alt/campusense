...
import { IncomeHead as IncomeHeadEntity } from '@/entities/IncomeHead';
...
export default function IncomeHead() {
  ...
  useEffect(() => {
    filterHeads();
  }, [incomeHeads, searchTerm, filterStatus]);

  const loadHeads = async () => {
    try {
      setIsLoading(true);
      const data = await IncomeHeadEntity.list('-created_date');
      setIncomeHeads(data);
    } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (formData.id) {
        await IncomeHeadEntity.update(formData.id, {
          name: formData.name,
        ...
        await IncomeHeadEntity.create(formData);
      ...
      alert('Failed to save income head.');
    }
  };
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
      ...
      <div className="lg:col-span-1">
        <Card>
          <CardHeader>
            <CardTitle>{formData.id ? 'Edit Income Head' : 'Create Income Head'}</CardTitle>
          </CardHeader>
          <CardContent>
      ...
      <div className="lg:col-span-2">
        <Card>
          <CardHeader>
            <CardTitle>Income Head Management</CardTitle>
          </CardHeader>
          <CardContent>
